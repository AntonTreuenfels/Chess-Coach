...
def setCastle(directions=None):
	_FEN.canCastle = CK.FEN_NONE if directions is None else directions
...
def setenPassant(square=None):
	_FEN.enPassant = CK.FEN_NONE if square is None else square
...
def getRecord():
	'''return complete fen record'''
	return _FEN.fenRecord.copy()
...
def lastRepeated():
	...
	reps = [ True for fen in _FEN.fenRecord if fen.startswith(repfen) ]
...
# these matter only if playing back recorded moves

def lastmoveBad(allfens):
	'''does last move match what was recorded ?'''
	if allfens is None:
		return False

	# what move are we on ?
	ndx = len( _FEN.fenRecord )
	return _FEN.fenRecord[ -1 ] != allfens[ ndx-1 ]

def canmoveNext(allfens):
	'''record short enough to advance to next move ?'''
	if allfens is None:
		return False

	return len( _FEN.fenRecord ) < len( allfens )

def moreMoves(allfens):
