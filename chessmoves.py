...
def makeMove(board, dstfile, dstrank, srcfile, srcrank):
	...
	# - "Px" and "px" = enpassant, "P?-" and ""p?-" = promote
		...
		match move:
			...
			case "p2":
				if ( rank == 7
					...
					canmove.append ( move )
			# can pawn capture left diagonally ?
			case "nw" | "sw" if file > 1:
				if occupiedBy(board, file-1, rank+dltrank) != CK.SQ_EMPTY:
					canmove.append( move )
				# enpassant capture possible ?
				elif FEN.getenPassant() == fr2sq(file-1, rank+dltrank):
					if (rank == 5 and move == "nw") or (rank == 4 and move == "sw"):
						canmove.append( move )
			# can pawn capture right diagonally ?
			case "ne" | "se" if file < 8:
				if occupiedBy(board, file+1, rank+dltrank) != CK.SQ_EMPTY:
					canmove.append( move )
				# enpassant capture possible ?
				elif FEN.getenPassant() == fr2sq(file+1, rank+dltrank):
					if (rank == 5 and move =="ne") or (rank == 4 and move == "se"):
						canmove.append( move )
			case _:
				pass
...
def kingMove(board, piece, file, rank, moves):
	...
	for move in moves:
		...
		match move:
			...
			case _:
				if occupiedBy(board, file+dltfile, rank+dltrank) != CK.SQ_OFFBOARD:
					canmove.append( move )
...
def allMoves(board):
	...
	for file in range(1,9):
		for rank in range(1, 9):
			...
			if occupant != CK.SQ_EMPTY:
				...
				moves[ square ].extend( canmove )

	# update reach counts
	whitecnt, blackcnt = canReach( board, moves )

	# at this point
	# - moves is a dictionary indexed by aquare that holds all possible moves from each square
	...
	return ( moves, whitecnt, blackcnt )
...
def kingChecked(files, rank, verboten):
	for file in files:
		if bool(verboten[file][rank]):
			return True
...
def potentialMoves(board):
	...
	for square, moves in allsquares.items():
		...
		startfile, startrank = findKing( board, friendlyking )
		canmove = list()
		for move in moves:
			...
			if dstpiece == enemyking:
				canmove.append( move )
				continue
			# assume friendly king does not move
			kingfile, kingrank = startfile, startrank
			...
			if occupant in "Kk":
				...
				match dstmove:
					case "ck":
						if kingChecked([srcfile, srcfile+1], srcrank, verboten):
							continue
					case "cq":
						if kingChecked([srcfile, srcfile-1], srcrank, verboten):
							continue
					case _:
						if occupant == friendlyking:
							kingfile, kingrank = dstfile, dstrank
...
def getInsufficient(board):
	...
	def insufficient(bcount, ncount, pcount):
		...
		# - always insufficient if true, but may be insufficient if not true
		return pcount < 1
...
def tactic(board, piece, square, allsquares, whites, blacks):
	...
	attval = valueOf( piece )
	att_color = colorOf( piece )
	sqfile, sqrank = sq2fr( square )
	attacker, defender, enemyking = (whites, blacks, "k") if isWhite(att_color) else (blacks, whites, "K")

	# is the enemy king in check (could be discovered check) ?
	kgfile, kgrank = findKing( board, enemyking )
	match attacker[kgfile][kgrank]:
		case 0:
			pass
		case 1:
			result |= CK.OC_CHECK
	...
	for direction, file, rank in allsquares[square]:
		...
		if attval <= valueOf(occupant) or  attnum > defnum:
		...
		# pawns on the same file as attacker may be skewered or pinned
		# if they can capture diagonally (which they can't)
		if occupant in "Pp" and file == sqfile:
			blocked = True
			# where can the pawn go to from the square it's on ?
			pwnsquare = fr2sq( file, rank )
			for pwndir, pwnfil, pwnrnk in allsquares[pwnsquare]:
				if pwndir in ["nw", "ne", "sw", "se"]:
					blocked = False
			# pawn can't capture diagonally ?
			if blocked:
				continue
		...
		if colorOf(nxtoccupant) == colorOf(occupant):
			obsval = valueOf( nxtoccupant )
			# is the obscured piece worth more than attacker ?
			# - or would it be overwhelmed if the defender moved ?
			if (obsval > attval) or (attacker[nxtfile][nxtrank]+1 > defender[nxtfile][nxtrank]):
				# a pin ?
				...
				# - practically, we will just say anything that is not a pin is a skewer
				else:	# defval >= obsval
					result |= CK.OC_SKEWER
	...
	match forkable:
		...
		case _:
			...
			if attacked == 1 and not (result & CK.OC_INCHECK):
				result |= CK.OC_ATTACK
...
def getNextMove(begboard, recordfens):
	'''pieces and squares of next recorded move'''
	# it happens that all actual fens match recorded fens to this point
	# - because if they didn't, we already flagged that as an error
	# - so the only reason we need the actual fens is to know where we are in the sequence
	actualfens = FEN.getRecord()
	ndx = len( actualfens )
	endboard = FEN.putFen( recordfens[ndx] )
	for file in range(1, 9):
		for rank in range(1, 9):
			befocc = occupiedBy( begboard, file, rank )
			aftocc = occupiedBy( endboard, file, rank )
			# this should happen twice over entire board
			if befocc != aftocc:
				# this piece moved ?
				if aftocc == CK.SQ_EMPTY:
					srcpiece = befocc
					srcfile = file
					srcrank = rank
				# this square was moved to
				else:
					dstfile = file
					dstrank = rank

	board, captured = makeMove( begboard, dstfile, dstrank, srcfile, srcrank )

	# make sure fen variables match what we had to start
	_ = FEN.putFen( actualfens[-1] )

	return board, ( srcpiece, captured ), ( fr2sq(srcfile, srcrank), fr2sq(dstfile, dstrank) )
